...
const ReviewModal = memo(({
  ...
  const handleRatingChange: RatingChangeHandler = useCallback((rating: number) => {
    setReviewForm((prev: typeof reviewForm) => ({ ...prev, review_ratings: rating }));
  }, [setReviewForm]);
  ...
  const isFormValid = reviewForm.review_message.trim() && reviewForm.review_ratings > 0;
  ...
  return (
    <Dialog open={isReviewModalOpen} onOpenChange={setIsReviewModalOpen}>
      <DialogContent className="sm:max-w-[620px] h-[90dvh] md:h-[95dvh] bg-gradient-to-br from-blue-50 via-white to-indigo-50 border-0 shadow-2xl overflow-scroll md:overflow-hidden">
        ...
        <div className="absolute hidden md:block top-0 left-0 right-0 h-24 bg-gradient-to-r from-blue-500 to-indigo-600 opacity-10"></div>
        
        <DialogHeader className="relative z-10 text-center pb-2">
       
          
          <DialogTitle className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-blue-700 to-indigo-800 bg-clip-text text-transparent">
            ✨ Share Your Experience
          </DialogTitle>
          
          <DialogDescription className="text-base  text-gray-600  leading-relaxed">
            Help others by sharing your experience with{' '}
            <span className="font-semibold text-blue-700">{service?.service_name}</span> treatment
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="bg-white rounded-2xl p-6 border border-blue-100 shadow-sm">
              <div className="flex flex-col items-center space-y-4">
                <div className="flex items-center justify-center gap-2 p-3 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl">
                  {renderStars(reviewForm.review_ratings, true, handleRatingChange)}
                </div>
                ...
                {reviewForm.review_ratings > 0 ? (
                  <div className="flex items-center gap-2 animate-fade-in">
                    ...
                    <span className={`text-lg font-bold ${getRatingText(reviewForm.review_ratings)?.color}`}>
                      {getRatingText(reviewForm.review_ratings)?.text} ({reviewForm.review_ratings}/5)
                    </span>
                  </div>
                ) : (
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <Textarea
                ...
                className="min-h-[120px] bg-white border-2 border-blue-100 focus:border-blue-400 focus:ring-blue-200 rounded-xl text-base leading-relaxed resize-none transition-all duration-300"
                maxLength={500}
              />
              ...
              <div className="flex justify-between items-center mt-2">
                <p className="text-xs text-gray-500">
                  💡 The more details you share, the more helpful your review becomes
                </p>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-3 pt-6 border-t border-blue-100">
          ...
          <Button
            ...
            className={`w-full sm:w-auto font-bold py-3 px-8 rounded-xl transition-all duration-300 transform ${
              isFormValid 
